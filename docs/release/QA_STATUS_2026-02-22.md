...
2. `flutter test`: PASSED
   ...
   - widget smoke test에서 `app_state_initialize` 오류 로그 미발생
...
2. 릴리즈 블로커는 코드 품질이 아니라 운영 검증(실기기 QA, RLS 실행, Android release 서명)임.
...
3. Android release signing config 적용 (`apps/mobile/android/app/build.gradle.kts:35`).
4. 런북 순서대로 내부 배포 리허설 수행 (`docs/release/TESTFLIGHT_INTERNAL_RELEASE.md`).
